    ...
    Txt.fn.bindPasteFilter = function () {
        ...
        $txt.on('paste', function (e) {
            ...
            if (editor.config.noPaste) {
                alert ('Copying and pasting is prohibited!');
                return false;
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        var $row = $('<span>0</span>');
        var $rowspan = $('<span> Row </span>');
        var $col = $('<span>0</span>');
        var $colspan = $('<span> Column </span>');
        var $tr;
        var i, j;
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        // tab
        var $uploadTab = $('<a href="#">Upload Image</a>');
        var $linkTab = $('<a href="#">Image from Network</a>');
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        $btnSubmit.click(function (e) {
            ...
            if (!reg.test(link)) {
                alert('Format of Video link error!');
                menu.dropPanel.focusFirstInput();
                return;
            }

            if (isNaN(width) || isNaN(height)) {
                alert('Width and height are not numbers!');
                return;
            }
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        fns.onload || (fns.onload = function (resultText, xhr) {
            ...
            if (!resultText.success) {
                ...
                img.onerror = function () {
                    E.error('Error in obtaining picture via returned result:' + resultText.bizContent);
                    img = null;
                };
        ...
        fns.ontimeout || (fns.ontimeout = function (xhr) {
            E.error('上传图片超时');
            alert('Picture upload timeout');
        });

        // -------- 定义error函数 --------
        fns.onerror || (fns.onerror = function (xhr) {
            E.error('上传上图片发生错误');
            alert('An error occurred when uploading the picture');
        });
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        if (!customUpload) {
            ...
            alert('Invalid custom uploading, refer to browser log at console.log for details');
